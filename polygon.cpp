...
void Polygon::calculateEdges(bool useDDA)
{
  for (int i = 0; i < 3; i++)
  {
    for (int edgeOrderIndex = 0; edgeOrderIndex < numTriangles; edgeOrderIndex++)
    {
      for (int j = 0; j < 3; j++)
      {
        int indexA;
        ...
        if (j != 2)
        {
          indexA = getElement(triangles[edgeOrderIndex], j);
          indexB = getElement(triangles[edgeOrderIndex], j+1);
        }
        else
        {
          indexA = getElement(triangles[edgeOrderIndex], j);
          indexB = getElement(triangles[edgeOrderIndex], 0);
        }
...
void Polygon::recalculateEdges(bool useDDA)
{
  edgesXY.clear();
  edgesXZ.clear();
  edgesYZ.clear();
  calculateEdges(useDDA);
}
